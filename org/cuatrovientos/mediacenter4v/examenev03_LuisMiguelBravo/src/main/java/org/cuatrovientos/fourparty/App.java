	...
	private static void doAction(int option, MediaCenter player, FileType fileType) {
		...
		case 1:
			...
			if (fileType.equals(FileType.SONG)) {
				...
				cancion = new Song(title, author, duration);
				file.FILE_TYPE = fileType;
			}
			else{
				String director = getDirector();
				String gender = getGender();
				contenido = new Film(title, duration, director, gender);
				file.FILE_TYPE = fileType;
			}		
			player.new_FILE(cancion);
			break;
		case 2:
			title = getTitle();
			player.Remo_FILE(title);;
			break;
		case 3:
			title = getTitle();
			System.out.println(player.SeARch(title));
			break;
		case 4:
			player.Show_ALL();
			break;
